...
const viewProfile = async (req, res) => {
    ...
    db.query(findQuery, [id], (err, results) => {
        ...
        logger.info(message.RETRIEVED_USER)
        return res
            .status(StatusCodes.OK)
            .json(
                new GeneralResponse(
...
const resetPassword = async (req, res) => {
    ...
    db.query(query, [id], async (err, result) => {
        ...
        db.query(updateQuery, [hashedNewPassword, id], (err, result) => {
            ...
            if (result.affectedRows === 0) {
                logger.error(message.NOT_FOUND)
                return res
                    .status(StatusCodes.NOT_FOUND)
                    .json(
                        new GeneralResponse(
...
const send_Mail_Of_OTP = async (req, res) => {
    try {
        ...
        otpTable.db.query(query, [email], async (err, result) => {
            ...
            if (result.length === 0) {
                logger.error(message.OTP_ERROR_EMAIL)
                return res
                    .status(StatusCodes.NOT_FOUND)
                    .json(
                        new GeneralResponse(
            ...
            db.query(otpQuery, [userId, otp], async (otpErr) => {
                if (otpErr) {
                    logger.error(message.INTERNAL_SERVER_ERROR,otpErr);
                    return interServerError(res, message.INTERNAL_SERVER_ERROR);
                }

                try {
                    const subject = 'OTP for password reset';
                    const text = `Your OTP for resetting the password is: ${otp}. It is valid for 2 minutes.`;
                    ...
                    logger.info(message.OTP_SEND_SUCCESS)
                    return res
                        .status(StatusCodes.OK)
                        .json(
                            new GeneralResponse(
...
const forgotPassword = async (req, res) => {
    try {
        ...
        otpTable.db.query(query, [email], async (err, result) => {
            ...
            if (result.length === 0) {
                logger.error(message.OTP_NOT_FOUND)
            ...
            if (otp !== storedOtp) {
                logger.error(message.INVALID_OTP)
            ...
            db.query(updatePasswordQuery, [hashedPassword, email], (updateErr) => {
                ...
                logger.info(message.PASSWORD_RESET_SUCCESS)
                return res
                    .status(StatusCodes.OK)
                    .json(
                        new GeneralResponse(
        ...
        logger.info(message.INTERNAL_SERVER_ERROR)
        return interServerError(res, message.INTERNAL_SERVER_ERROR);
    }
};
